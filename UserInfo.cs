...
namespace DAL
{
    public class UserInfo
    {
        ...
        public IEnumerable<Model.UserinfoNo> GetPage(Model.Page page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "with a as(select row_number() over(order by username) as num,userinfo.* form userinfo)";
                sql += "select * from where num between(@pageIndex-1)*@pageSize+1 and @pageIndex*@pageSize;";
        ...
        public int Add(Model.UserInfo user)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into userinfo values(@userNanme,@passWord,@qq,@email,@type,@userImg);";
